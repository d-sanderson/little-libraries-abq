...
const MainLayout = async (props) => {
  const { results: libraries } = await props.context.env.DB.prepare("SELECT * FROM LittleLibraries").all();
      ...
      <main class="relative flex items-start min-h-screen 2xl:px-0 lg:px-4 px-5 pt-0 pb-0 mx-auto mt-1 max-w-8xl rounded-t-3xl">
        <aside class="fixed block w-56 h-screen max-h-screen pt-20 pb-5 pr-5 overflow-scroll bg-white lg:pt-8 lg:pb-16 z-20 top-[55px]">
          ...
          <div class="relative grid grid-flow-row text-sm mb-7 auto-rows-max">
            {libraries.map((lib) => (
              <Link class={`group flex w-full items-center rounded-md border px-2 py-1.5 ${path.startsWith(`/projects/${lib.id}`) ? 'bg-gray-100 border-gray-200/60' : 'border-transparent'}`} to={`/projects/${lib.id}/view`}>{lib.name}</Link>
            ))}
          </div>
        </aside>
